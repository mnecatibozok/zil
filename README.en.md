...
10. [RF Remote Control (Physical)](#-rf-remote-control-physical)
11. [📱 Connecting From a Phone / Mobile Device (Remote Control)](#-connecting-from-a-phone--mobile-device-remote-control)
12. [Sound Files](#-sound-files)
13. [Earthquake Drill Mode](#-earthquake-drill-mode)
14. [Prayer Time Integration](#-prayer-time-integration)
15. [FAQ](#-faq)
...
- **💾 Settings Backup / Restore**: exports ALL settings — school info, bell schedule, quiet-mode calendar, sound settings, prayer/amplifier settings, and more — into a single JSON file (**📤 Export All Settings**), or restores from that file (**📥 Restore From Backup**). Use this when moving to another computer/Raspberry Pi or for disaster recovery.
...
- **Auto Shutdown**: safely shuts the computer down at a time you specify (an amp-off signal is sent first, then the server issues a `shutdown` command a few seconds later). Only works while the program runs through the local server (`zil-baslat.bat`). It has two modes: **🔁 Bulk Time** — enter one time and every day shuts down at that time (the previous behavior). **📅 By Day** — Monday through Sunday, each day gets its own **active/inactive** checkbox and its own **hour/minute** picker (e.g. 17:00 on weekdays, shutdown off on weekends). The status line ("⏻ Shutdown: 17:00 — active") reflects whichever mode is selected; in By Day mode it shows today's setting.
...
> 💡 This RF remote is a **separate physical device from the phone remote
> below** — it needs no Wi-Fi/network and only works at short range. The two
> don't replace each other; you can use both depending on the situation.

---

## 📱 Connecting From a Phone / Mobile Device (Remote Control)

A teacher or administrator can trigger the bell, the anthem, or a moment of
silence **from a phone's browser** while out in the hallway or the
schoolyard — no app to install, any browser will do.
...
[Phone — any browser]
         ...
         ▼
http://<computer's-local-IP>:<port>/kumanda
         │
         ▼
[Python server — zunucu/handler.py]
   ├── Is the PIN correct? → issue a session cookie
   ├── Store the button command in memory (POST /api/remote)
         │
         ▼  (the Chrome tab on the school computer polls once a second)
[zil.html — Chrome on the school computer, GET /api/remote-poll]
         │
         ▼
executeRemoteCmd(cmd) → manualPlay() / stopAll()
         │
         ▼
The sound actually plays from the school computer, through the amplifier
```

**Important:** The phone **never plays sound directly**. It only sends a
"play this" command to the school computer; the sound is always played (and
the amp triggered) by the `zil.html` tab on the school computer. Because of
this:

- The program (Chrome tab) on the school computer **must be open and
  running**. If the program is closed, a command sent from the phone has no
  effect.
- The phone only works while it's on **the same local network (same
  Wi-Fi)**. It **cannot connect over the internet** from a different network
  (e.g. mobile data at home) unless port forwarding has been set up — this
  is a local-network feature, not something exposed to the public internet.
- Because the server listens on `0.0.0.0`, it's reachable from any device on
  the network — so `/kumanda` can also be opened from another computer's
  browser, not just a phone. Think of it as "remote control from any browser
  on the network," not a phone-only feature.

### Step by step

**1) Find the address and PIN**

When the program is started (`zil-baslat.bat`), the black console window
that opens shows lines like this:

```
[ZIL] Network : http://192.168.1.10:8765/zil.html
[ZIL] Remote  : http://192.168.1.10:8765/kumanda
[ZIL] Kumanda PIN : 384729  (can be changed via kumanda-pin.json)
```

- `192.168.1.10` and `8765` will differ based on your network and the
  current port.
- **The PIN stays the same across restarts** (it's randomly generated once
  and saved to `kumanda-pin.json`); edit that file directly if you want to
  change it.
- If the console window gets closed, restart the program to see this again
  (or open `kumanda-pin.json` directly).

**2) Connect from the phone**

Make sure the phone is **connected to the school's Wi-Fi**, then go to
`http://192.168.1.10:8765/kumanda` (using your own IP/port) in the browser
— Chrome, Safari, whichever.

**3) Enter the PIN**

A dark-themed, centered "🔒 Bell Remote — Enter PIN" screen appears. Enter
the 6-digit PIN and tap **Log In**. Once the PIN is correct, the session is
remembered for 24 hours — you won't be asked for the PIN again on that same
phone/browser during that time. (It's asked again once the browser data is
cleared or the 24 hours pass.)

**4) Use the remote screen**

Once logged in, five large touch buttons appear:

| Button | Effect |
|---|---|
| 🎖 **National Anthem** | Starts playing the anthem on the school computer |
| 🤲 **Tribute 1min** | Plays the 1-minute moment-of-silence sound |
| 🤲 **Tribute 2min** | Plays the 2-minute moment-of-silence sound |
| 🔔 **Ring Bell** | Plays the manual bell sound |
| ⏹ **STOP** | Immediately stops whatever is currently playing |

A **TR / EN** language toggle is available in the top corner; the choice is
remembered on that phone. After tapping a button, the screen briefly shows
"Sending..." followed by a "✅ ..." or "❌ ..." status message.

**5) What happens on the school computer**

Once the command reaches the `zil.html` tab on the school computer, an
entry is added to the on-screen activity log with a 📡 icon (e.g. "Remote:
..."), and the corresponding sound plays normally through the amplifier. If
the program is still starting up and the sound files haven't fully loaded
yet, the command isn't lost — it's queued and played as soon as the sounds
finish loading.

### Can more than one person use it at the same time?

Yes — the system has no separate user accounts, just a single PIN. Anyone
who knows the PIN (or has an active session in their browser) can use the
remote page. This means multiple administrators/teachers can connect with
the same PIN from different phones; it also means the PIN should be treated
like a shared password that shouldn't be spread outside the school.

### Security note

According to a developer comment in the server code, an **earlier version**
of this feature let **anyone on the network** reach `/kumanda` without a
password. The PIN + session-cookie layer was added later to close that gap.
In the current version, neither the remote buttons nor the `/api/remote`
command are accepted by the server without the correct PIN. Since anyone on
the school's Wi-Fi could theoretically try the `/kumanda` address (the
address itself isn't secret, only PIN-protected), avoid sharing the PIN
outside the school and consider changing it periodically via
`kumanda-pin.json`.

### Common issues (phone remote)

| Symptom | Likely Cause / Fix |
|---|---|
| Phone can't reach the address at all | The phone may be on a different Wi-Fi network or on mobile data; connect to the same network. A firewall on the school computer/network may be blocking the port. |
| I don't know the PIN | The program prints it in the console window (`[ZIL] Kumanda PIN : ...`); if the window is closed, open `kumanda-pin.json`. |
| I tap a button but no sound plays | Make sure the `zil.html` tab on the school computer is open and running — the phone doesn't play the sound, that tab does. Also make sure the browser's audio permission has been enabled once with a click on the computer. |
| "❌ Connection error" shown | The server (Python) may have stopped; check that the program is still running on the school computer. |
| PIN is asked again after 24 hours | This is expected — the session cookie (24h) has expired; just enter the PIN again. |
...
**Q: Can I control it from another computer or phone on the network?**
A: Yes. Go to `http://[server-ip]:[port]/kumanda` (e.g. `http://192.168.1.10:8765/kumanda`) and enter the 6-digit PIN shown in the console window. This works from any device's browser on the same Wi-Fi network, including phones. For full details on where to find the address/PIN and troubleshooting, see [📱 Connecting From a Phone / Mobile Device](#-connecting-from-a-phone--mobile-device-remote-control).
...
**September 7, 2026**
- **README: fully documented the Phone/Mobile Remote Control feature:** the `/kumanda` page, previously mentioned in one sentence in the FAQ, now has its own detailed section — covering where to find the address/PIN, step-by-step usage, a security note (an earlier version had no password; a PIN layer was added later), and common troubleshooting.

**September 1, 2026 (5)**
- **Auto Shutdown: per-day times plus a bulk mode:** Auto Shutdown on the Shutdown/Channel tab now has two modes: **🔁 Bulk Time** (one time applied to every day, as before) and **📅 By Day** (Monday through Sunday, each day has its own active/inactive checkbox and its own hour/minute picker — e.g. 17:00 on weekdays, shutdown off on weekends). Your previously saved single time is carried forward automatically to weekdays and pre-filled into the By Day table; behavior stays unchanged as long as you remain in Bulk Time mode.
- **Moved the Settings Backup/Restore card to the School/Language tab:** the "📤 Export All Settings" / "📥 Restore from Backup" buttons now live at the bottom of the 🏫 School/Language tab, right under the language switcher, instead of the Shutdown/Channel tab.
