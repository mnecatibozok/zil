...
class ZilHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _post_ses_yukle(self, body):
        """Ayarlar panelinden seçilen bir zil sesini zilsesleri/ klasörüne kalıcı yazar.
        Böylece bilgisayar kapanıp açılsa da, sayfa yenilense de seçim kaybolmaz —
        autoFetchSounds() bir dahaki açılışta dosyayı klasörden otomatik bulur."""
        try:
            filename = os.path.basename(body.get('filename', '') or '')
            data_b64 = body.get('data', '') or ''

            if not filename or not data_b64:
                return self._serve_json({'ok': False, 'hata': 'filename veya data eksik'})

            izinli_uzanti = ('.mp3', '.m4a', '.aac', '.ogg', '.wav', '.flac')
            if not filename.lower().endswith(izinli_uzanti):
                return self._serve_json({'ok': False, 'hata': 'Desteklenmeyen ses uzantısı'})

            try:
                raw_bytes = base64.b64decode(data_b64)
            except Exception:
                return self._serve_json({'ok': False, 'hata': 'Geçersiz base64 veri'})

            if len(raw_bytes) > 50 * 1024 * 1024:
                return self._serve_json({'ok': False, 'hata': 'Dosya çok büyük (>50MB)'})

            cwd     = os.path.realpath(os.getcwd())
            ses_dir = os.path.join(cwd, 'zilsesleri')
            os.makedirs(ses_dir, exist_ok=True)
            dest = os.path.join(ses_dir, filename)

            with open(dest, 'wb') as f:
                f.write(raw_bytes)

            print(f'[ZIL] Ses dosyası kalıcı kaydedildi: zilsesleri/{filename} ({len(raw_bytes)} bayt)')
            self._serve_json({'ok': True, 'file': filename, 'path': f'zilsesleri/{filename}'})
        except Exception as e:
            print(f'[ZIL] HATA — ses-yukle: {e}')
            self._serve_json({'ok': False, 'hata': str(e)})
