...
from watchdog  import ZilWatchdog
...
_kumanda_lock : threading.Lock = threading.Lock()   # ThreadingTCPServer için
...
class ZilHandler(http.server.SimpleHTTPRequestHandler):
    ...
    # watchdog bağlantısı — sunucu.py tarafından set edilir
    watchdog: ZilWatchdog | None = None
    ...
    def do_GET(self):
        ...
        if path == '/api/watchdog-durumu':
            return self._serve_json(self.watchdog.watchdog_durumu())
    ...
    def _get_ntp_kontrol(self):
        ...
        if offset is None:
            ...
            return
        uyari = abs(offset) >= 30  # 30 saniyeden fazla fark varsa uyar
        self._serve_json({
            'ok'    : True,
            'offset': offset,          # pozitif = sistem ileri, negatif = geri
            'uyari' : uyari,
    ...
    def do_POST(self):
        ...
        if path == '/api/handshake':
            return self._post_handshake(body)

        if path == '/api/program-guncelle':
            return self._post_program_guncelle(body)
    ...
    def _post_handshake(self, body):
        key = body.get('key', '')
        tip = body.get('tip', 'CALINDI')
        if key:
            self.watchdog.handshake_al(key, tip)
            self._serve_json({'ok': True})
        else:
            self._serve_json({'ok': False, 'hata': 'key eksik'})

    def _post_program_guncelle(self, body):
        ziller  = body.get('ziller', [])
        gun_idx = body.get('gunIdx', None)
        self.watchdog.program_guncelle(ziller, gun_idx)
        print(f'[WD] Program güncellendi: {len(ziller)} aktif zil (gunIdx={gun_idx})')
        self._serve_json({'ok': True, 'sayi': len(ziller)})

    def _post_pause(self, body):
        aktif = body.get('aktif', False)
        self.watchdog.pause_bildir(aktif)
        self._serve_json({'ok': True, 'paused': aktif})
    ...
    def _post_move_to_temp(self, body):
        ...
        # Path traversal koruması: kök dizin dışına çıkamaz
    ...
    def _post_restore_from_temp(self, body):
        ...
        if original_path:
            ...
            # Path traversal koruması: .origin dosyası manipüle edilmiş olabilir
    ...
    def _post_exit(self):
        ...
        def _exit():
            # Önce izole Chrome penceresini kapat (ZilSistemi profili)
            try:
                import subprocess
                subprocess.run(
                    ['wmic', 'process', 'where',
