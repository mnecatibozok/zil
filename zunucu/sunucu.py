...
import os, sys, socket, socketserver, threading

# ── Klasör içinden import ────────────────────────────────
# Bu dosya sunucu/ klasöründe. Modüller (utils, ezan, watchdog, handler)
...
def main() -> None:
    ...
    # Watchdog
    watchdog = ZilWatchdog()
    ZilHandler.watchdog = watchdog
    threading.Thread(target=watchdog.calistir, daemon=True, name='ZilWatchdog').start()
