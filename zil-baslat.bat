...
:: ONEMLI: Profil klasoru artik TEMP yerine program klasorunun icinde.
:: TEMP klasoru bazi okul/kiosk bilgisayarlarinda her yeniden baslatmada
:: (disk temizligi / antivirus / "oturum sifirlama" yazilimlari tarafindan)
:: otomatik temizlenebiliyor. Bu durumda Chrome profili ile birlikte
:: localStorage'daki TUM ayarlar (tablo goster/gizle, hafta sonu zili, vb.)
:: siliniyor ve program her acilista varsayilana donuyormus gibi gorunuyordu.
:: Profili "%~dp0" altina almak bu sorunu ortadan kaldirir.
set CHROME_PROFILE_DIR=%~dp0chrome-profil
start "" "%CHROME_EXE%" --kiosk "%ZIL_URL%" --user-data-dir="%CHROME_PROFILE_DIR%" --disable-session-crashed-bubble --disable-infobars --noerrdialogs --autoplay-policy=no-user-gesture-required --no-first-run --disable-translate
